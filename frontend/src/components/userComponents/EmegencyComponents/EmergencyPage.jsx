...
export default function EmergencyPage() {
  ...
  useEffect(() => {
    const fetchHotlines = async () => {
      try {
        const res = await axios.get("/api/emergency"); // Public GET request
    ...
    <div className="min-h-screen bg-[#f04e37] flex flex-col items-center text-sm relative px-4 md:px-0 text-white">
      <div className="w-full max-w-xl">
        ...
        <div className="mt-4 text-center">
          {loading ? (
            <p className="text-white opacity-70">Loading hotlines...</p>
          ) : hotlines.length > 0 ? (
            ...
            <p className="text-white opacity-70">No hotlines available.</p>
          )}
        </div>
      </div>
