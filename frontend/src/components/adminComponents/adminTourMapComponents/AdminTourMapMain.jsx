...
export default function AdminTourMapMain() {
  ...
  useEffect(() => {
    ...
    const ro = new ResizeObserver(() => {
      const m = adminMapRef.current?.getMap?.();
      if (m) m.resize();
    });
    ro.observe(el);
  ...
  const saveMask = async () => {
    ...
    try {
      ...
      if (drawRef.current && map) {
        ...
        if (data.features.length > 0) {
          ...
          notify(
            "error",
            "No Mask",
            "No mask found to save. Please draw a mask first."
          );
          setIsMaskingMode(false);
          return;
        }
      ...
      notify(
        "success",
        "Mask Saved",
        "The boundary mask has been saved successfully."
      );
    } catch (err) {
      console.error(err);
      notify(
        "error",
        "Save Failed",
        "Failed to save the boundary mask. Please try again."
      );
    } finally {
      exitMaskEditing();
    }
  ...
  const handleMapClick = (event) => {
    ...
    if (!isInsideMask(lng, lat)) {
      notify(
        "error",
        "Invalid Location",
        "Pin location is outside Intramuros bounds. Please place pins within the highlighted area."
      );
      return;
    }
  ...
  const addPinFromCoords = () => {
    ...
    if (!isInsideMask(lng, lat)) {
      notify(
        "error",
        "Invalid Location",
        "Coordinates are outside Intramuros bounds. Please enter valid coordinates within the highlighted area."
      );
      return;
    }
  ...
  const handleFormSubmit = (e, index) => {
    ...
    const categoryValue =
      typeof pin.category === "object" ? pin.category?._id : pin.category;
    if (!categoryValue) {
    ...
    if (pin.averageTimeSpent !== null && pin.averageTimeSpent !== undefined) {
      ...
      if (!Number.isFinite(ats) || ats < 1) {
        errors.averageTimeSpent =
          "Average time must be a positive number of minutes";
      }
    }
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          notify(
            "success",
            "Pin Saved",
            `Pin #${index + 1} has been saved successfully.`
          );
          setSelectedPin(null);
          setOriginalPinData(null);
          setIsAddingPin(false);
          ...
          notify(
            "error",
            "Save Failed",
            "Failed to save the pin. Please try again."
          );
  ...
  const handleArchive = (id) => {
    ...
    setConfirmModal({
      ...
      title: "Archive Pin?",
      message:
        "This pin will be moved to the archived section. You can restore it later if needed.",
      confirmText: "Archive",
      onConfirm: async () => {
        setConfirmModal((prev) => ({ ...prev, loading: true }));
        try {
          ...
          notify(
            "success",
            "Pin Archived",
            "The pin has been archived successfully."
          );
          setConfirmModal({
            isOpen: false,
            type: "warning",
          ...
          notify(
            "error",
            "Archive Failed",
            "Failed to archive the pin. Please try again."
          );
          setConfirmModal((prev) => ({ ...prev, loading: false }));
        }
      },
  ...
  const handleRestore = (id) => {
    ...
    setConfirmModal({
      ...
      title: "Restore Pin?",
      message:
        "This pin will be restored to the active pins list and will be available on the map again.",
      confirmText: "Restore",
      onConfirm: async () => {
        setConfirmModal((prev) => ({ ...prev, loading: true }));
        try {
          ...
          notify(
            "success",
            "Pin Restored",
            "The pin has been restored successfully."
          );
          setConfirmModal({
            isOpen: false,
            type: "warning",
          ...
          notify(
            "error",
            "Restore Failed",
            "Failed to restore the pin. Please try again."
          );
          setConfirmModal((prev) => ({ ...prev, loading: false }));
        }
      },
  ...
  const handlePermanentDelete = (id) => {
    ...
    setConfirmModal({
      ...
      message:
        "WARNING: This action cannot be undone! This pin and all its associated data will be permanently deleted from the database.",
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          notify(
            "success",
            "Pin Deleted",
            "The pin has been permanently deleted."
          );
          setConfirmModal({
            isOpen: false,
            type: "warning",
          ...
          notify(
            "error",
            "Delete Failed",
            "Failed to delete the pin. Please try again."
          );
          setConfirmModal((prev) => ({ ...prev, loading: false }));
        }
      },
  ...
  const handleCategorySubmit = async (e) => {
    ...
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          if (isEditing) {
            await api.put(`/admin/categories/${editCategoryId}`, {
              name: categoryName,
            });
            notify(
              "success",
  ...
  const handleCategoryDelete = (id) => {
    setConfirmModal({
      ...
      message:
        "This category will be permanently deleted. Pins using this category will have their category removed.",
  ...
  const handleGlbUpload = async (e, index) => {
    ...
    try {
      ...
      notify(
        "success",
        "Model Uploaded",
        "3D model uploaded successfully. Click Save Changes to apply."
      );
    } catch (err) {
      console.error(err);
      notify(
  ...
  const handleFacadeUpload = async (e, pinIndex) => {
    ...
    try {
      ...
      setPins((prev) =>
        prev.map((p, i) =>
          i === pinIndex ? { ...p, facadeUrl: uploadedUrl } : p
        )
      );
      notify(
        "success",
        "Facade Uploaded",
        "Facade image uploaded successfully. Click Save Changes to apply."
      );
    } catch (err) {
      console.error(err);
      notify(
        "error",
        "Upload Failed",
        "Failed to upload the facade image. Please try again."
      );
    }
  };
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          notify(
            "success",
            "Facade Deleted",
            "The facade image has been deleted successfully."
          );
        } catch (error) {
          ...
          notify(
            "error",
            "Delete Failed",
            "Failed to delete the facade image. Please try again."
          );
        }
      },
    });
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          notify(
            "success",
            "Model Deleted",
            "The 3D model has been deleted successfully."
          );
        } catch (error) {
          ...
          notify(
            "error",
            "Delete Failed",
            "Failed to delete the 3D model. Please try again."
          );
        }
      },
    });
  ...
  const handleMediaUpload = async (e, index) => {
    ...
    if (oversizedFiles.length > 0) {
      ...
      notify(
        "error",
        "File Too Large",
        `File(s) too large: ${fileNames}. Maximum size is 50MB per file.`
      );
      return;
    }
    ...
    try {
      ...
      notify(
        "success",
        "Files Uploaded",
        `${uploadedFiles.length} file(s) uploaded successfully. Click Save Changes to apply.`
      );
    } catch (err) {
      console.error(err);
      notify(
  ...
  return (
    <>
      ...
      <div className="flex items-stretch mt-8 mx-5 p-0 bg-gray-100 min-h-screen">
        <div
          ref={mapContainerRef}
          className="relative w-full h-[90vh] bg-white rounded-none md:rounded-2xl shadow-lg overflow-hidden"
        >
          {loading && (
              ...
              <Marker
                ...
                onDragEnd={async (event) => {
                  ...
                  if (!isInsideMask(lng, lat)) {
                    notify(
                      "error",
                      "Pin must be placed within Intramuros bounds"
                    );
          ...
          <div className="absolute top-6 right-6 z-[9999] flex items-end space-x-3">
            ...
            <div className="flex flex-col items-end space-y-2">
              <div className="bg-white rounded-lg shadow-md flex flex-col overflow-hidden relative z-[9999]">
                ...
                >
                  {isAddingPin ? (
                    <MapPinned className="w-5 h-5 mx-auto" />
                  ) : (
                ...
                >
                  {isMaskingMode ? (
                    ...
                    <Crop className="w-5 h-5 mx-auto" />
                  )}
                </button>
                <button
                ...
                <button
                  ...
                  style={
                    showCategoryPanel ? { backgroundColor: "#f04e37" } : {}
                  }
                >
                  ...
                  {activeTab === "active" ? (
                    (() => {
                      ...
                      const filteredPins = pins.filter((pin) => {
                        // Search filter
                        const matchesSearch = pin.siteName
                          ?.toLowerCase()
                        ...
                        const pinCategoryId =
                          typeof pin.category === "object"
                            ? pin.category?._id
                            : pin.category;
                        const matchesCategory =
                          filterCategory === "all" ||
                          pinCategoryId === filterCategory;
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {filteredPins.map((pin, index) => (
                            <div
                              ...
                              onMouseEnter={(e) =>
                                (e.currentTarget.style.borderColor = "#f04e37")
                              }
                              onMouseLeave={(e) =>
                                ...
                                <div className="flex gap-2 mt-auto">
                                  <button
                                    onClick={() => {
                                      ...
                                      const actualIndex = pins.findIndex(
                                        (p) => p._id === pin._id
                                      );
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <input
                        ...
                        onFocus={(e) => {
                          e.target.style.borderColor = "#f04e37";
                          e.target.style.boxShadow =
                            "0 0 0 3px rgba(240, 78, 55, 0.1)";
                        }}
                        onBlur={(e) => {
                      ...
                      <button
                        ...
                        onMouseLeave={(e) =>
                          (e.currentTarget.style.backgroundColor = "#f04e37")
                        }
                      >
                        {editCategoryId ? (
                          <Check size={16} />
