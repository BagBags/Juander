...
class ModelErrorBoundary extends Component {
  ...
  componentDidCatch(error, errorInfo) {
    console.log(
      "3D model failed to load in pin card, skipping:",
      error.message
    );
  }
...
const AdminPinCard = ({
  ...
  useEffect(() => {
    ...
    if (pin.siteDescription) {
      const sections = pin.siteDescription
        .split("\n\n")
        .filter((s) => s.trim());
    ...
    if (pin.siteDescriptionTagalog) {
      const sections = pin.siteDescriptionTagalog
        .split("\n\n")
        .filter((s) => s.trim());
        ...
        >
          ...
          <div className="relative category-dropdown-container">
            ...
            <div className="relative">
              ...
              {showCategoryDropdown && (
                <div
                  className="absolute z-50 w-full mt-2 bg-white border-2 rounded-xl shadow-lg max-h-64 overflow-hidden flex flex-col"
                  style={{ borderColor: "#f9c5bd" }}
                >
          ...
          <div>
                      ...
                      <div className="flex items-start gap-2">
                        <div className="flex-1">
                          ...
                          <textarea
                            value={section}
                            onChange={(e) =>
                              updateTagalogSection(index, e.target.value)
                            }
                            className="w-full border border-gray-300 rounded-xl p-3 text-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200"
                            rows="4"
                            maxLength={1000}
            ...
            <div className="flex gap-2 mt-3">
              ...
              >
                ...
                English:{" "}
                {(pin.siteDescription || "").trim()
                  ? `${descriptionSections.length} section(s)`
                  : "Empty"}
              </div>
              <div
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium ${
              ...
              >
                ...
                Tagalog:{" "}
                {(pin.siteDescriptionTagalog || "").trim()
                  ? `${tagalogSections.length} sections`
                  : "Empty"}
              </div>
            </div>
          ...
          <div className="border-t border-gray-200 pt-4 mt-4">
            ...
            <div className="flex flex-col space-y-3">
              <div className="relative">
                ...
                />
                <div
                  className={`border-2 border-dashed rounded-xl p-4 text-center hover:border-blue-400 transition-colors duration-200 ${
                    ...
                    return (
                      ...
                      >
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            handleRemoveMedia(selectedPinIndex, index)
                          }
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-2">
              Enable AR experience for this site. Tourists will be able to scan
              a QR code to access AR features.
            </p>
          </div>
              ...
              <div className="mt-3 space-y-3">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  ...
                  <p className="text-xs text-blue-600">
                    Discounted price applies to Students, PWD (Persons with
                    Disabilities), and Senior Citizens. Leave blank if no
          ...
          <div>
            ...
            <input
              ...
              max="120"
              step="1"
              value={pin.averageTimeSpent ?? ""}
              onChange={(e) => {
                const value =
                  e.target.value === "" ? null : Number(e.target.value);
                updatePinField(selectedPinIndex, "averageTimeSpent", value);
                if (setValidationErrors) {
                  setValidationErrors((prev) => ({
                    ...prev,
                    averageTimeSpent: "",
            ...
            />
            {validationErrors.averageTimeSpent && (
              <p className="text-red-500 text-sm mt-1">
                {validationErrors.averageTimeSpent}
              </p>
            )}
