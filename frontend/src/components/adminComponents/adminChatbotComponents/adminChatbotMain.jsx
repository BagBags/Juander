...
export default function AdminChatbot() {
  ...
  const loadMoreArchivedRef = useRef(null);
  const LOAD_STEP = 10;
  ...
  const fetchArchivedEntries = async () => {
    try {
      const res = await axios.get(`${API_BASE}/archived`, {
  ...
  const handleArchive = (id) => {
    setConfirmModal({
      ...
      message:
        "This entry will be moved to the archived section. Users will no longer receive this information when they ask related questions.",
  ...
  const handleRestore = (id) => {
    setConfirmModal({
      ...
      message:
        "This entry will be restored to the active knowledge base. Users will be able to receive this information again.",
  ...
  const handlePermanentDelete = (id) => {
    setConfirmModal({
      ...
      message:
        "This chatbot entry will be permanently deleted and cannot be recovered. This action cannot be undone.",
  ...
  const handleTagSubmit = async (e) => {
    ...
    try {
      if (editTagId) {
        ...
        await axios.post(TAG_API_BASE, { name }, { headers: getAuthHeader() });
      }
      setTagName("");
      fetchTags();
  ...
  const handleTagDelete = (id) => {
    setConfirmModal({
      ...
      title: "Delete Tag?",
      message:
        "This tag will be permanently deleted. All chatbot entries using this tag will need to be updated.",
      confirmText: "Delete Tag",
      onConfirm: async () => {
        setConfirmModal((prev) => ({ ...prev, loading: true }));
        try {
          await axios.delete(`${TAG_API_BASE}/${id}`, {
            headers: getAuthHeader(),
          });
          fetchTags();
          setConfirmModal({
  ...
  useEffect(() => {
    setVisibleCount(10);
  }, [filterTags, activeTab, entries.length]);

  useEffect(() => {
    setVisibleArchivedCount(10);
  }, [activeTab, archivedEntries.length]);
    ...
    const observer = new IntersectionObserver(
      (entriesObs) => {
        ...
        if (entry.isIntersecting) {
          setVisibleCount((v) =>
            Math.min(v + LOAD_STEP, filteredEntries.length)
          );
        }
      },
      { threshold: 0.1 }
  ...
  useEffect(() => {
    ...
    const node = loadMoreArchivedRef.current;
    if (!node) return;
    const observer = new IntersectionObserver(
      (entriesObs) => {
        const entry = entriesObs[0];
        if (entry.isIntersecting) {
          setVisibleArchivedCount((v) =>
            Math.min(v + LOAD_STEP, archivedEntries.length)
          );
        }
      },
      { threshold: 0.1 }
    ...
    <>
      ...
      <section className="bg-gray-50 min-h-screen">
        <div className="flex flex-col lg:flex-row items-stretch gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6">
              ...
              <div className="flex gap-2 mb-6">
                <button
                  ...
                  style={
                    activeTab === "active" ? { backgroundColor: "#f04e37" } : {}
                  }
                >
                ...
                filteredEntries.length > visibleCount && (
                  <div className="flex items-center justify-center mt-4">
                    <button
                      ...
                      className="px-4 py-2 bg-[#f04e37] text-white rounded-lg hover:bg-[#e03d2d] transition"
                    >
                      Load more
                    </button>
                  </div>
              ...
              {activeTab === "archived" &&
                archivedEntries.slice(0, visibleArchivedCount).map((entry) => (
                  <div
                    key={entry._id}
              ...
              {activeTab === "archived" &&
                archivedEntries.length > visibleArchivedCount && (
                  <div className="flex items-center justify-center mt-4">
                    <button
                      onClick={() =>
                        setVisibleArchivedCount((v) =>
              ...
              {activeTab === "archived" && (
                <div ref={loadMoreArchivedRef} className="h-6"></div>
              )}
          ...
          <div
            className="w-full lg:flex-1 lg:min-w-0 space-y-6 lg:sticky lg:top-6 self-start 
                max-h-[calc(100vh-3rem)] overflow-y-auto md:pr-2"
          >
            {/* Entry Form */}
            <div className="bg-white p-6 rounded-2xl shadow-lg">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  ...
                  <div
                    className={`grid grid-cols-1 gap-2 max-h-48 overflow-y-auto p-2 border-2 rounded-lg ${
